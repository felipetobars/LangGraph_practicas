...
class Feedback(BaseModel):
    is_funny: bool = Field(description="Decide if the joke is funny or not. Return True if it is, False otherwise.")
    feedback: str = Field(description="If the joke is not funny, provide feedback on how to improve it.")

llm = ChatOllama(model="qwen2.5:7b", temperature=0)
ll_evaluator = llm.with_structured_output(schema=Feedback)

SYSTEM_PROMPT = """
Un chiste gracioso debe ser más de 2 párrafos, puede ser de humor negro, esos son los mejores.
"""
